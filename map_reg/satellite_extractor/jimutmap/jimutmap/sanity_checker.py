...
def create_sanity_db(min_lat_deg, max_lat_deg, min_lon_deg, max_lon_deg,sanity_obj,
                     latLonResolution=0.0005, verbose=False):
    ...
    for i in tqdm(np.arange(min_lat_deg, max_lat_deg, latLonResolution)):
        for j in np.arange(min_lon_deg, max_lon_deg, latLonResolution):
            ...
            # write the query
            query_insert = "INSERT OR IGNORE INTO sanity VALUES ('{}','{}','{}','{}','{}')".format(
                key_id, xTile, yTile, 0, 0)
...
def update_sanity_db(folder_name):
    ...
    for tile_name in tqdm(all_files_folder):
        if tile_name.count('_') == 1:
            ...
            # print(create_id)
            cur.execute(
                '''UPDATE sanity SET satellite_tile = 1 WHERE id = ?''',
                (str(create_id),))  # set the satellite_tile to 1
        ...
        if tile_name.count('_') == 2:
            ...
            create_id = str(xTile_val) + "_" + str(yTile_val)
            # print(create_id)
            cur.execute('''UPDATE sanity SET road_tile = 1 WHERE id = ?''',
                        (str(create_id),))  # set the road_tile to 1
    con.commit()
...
def shall_stop():
    ...
    get_sat_0s = cur.execute(
        ''' SELECT * FROM sanity WHERE satellite_tile = 0 ''')
    get_sat_0s_val = cur.fetchall()  # converts the cursor object to number
    total_number_of_sat0s = len(get_sat_0s_val)
    print("Total number of satellite images needed to be downloaded = ",
          total_number_of_sat0s)
...
def get_sat_img_id():
    ...
    get_sat_0s = cur.execute(
        ''' SELECT id FROM sanity WHERE satellite_tile = 0 ''')
    get_sat_0s_val = cur.fetchall()  # converts the cursor object to number
...
def sanity_check(min_lat_deg, max_lat_deg, min_lon_deg, max_lon_deg, zoom,
                 verbose, threads_, container_dir, sanity_obj):
    ...
    batch = 1
    create_sanity_db(min_lat_deg, max_lat_deg, min_lon_deg, max_lon_deg,
                     latLonResolution=0.0005, verbose=False, sanity_obj=sanity_obj)
    generate_summary()
    ...
    while (shall_stop() == 0):
        ...
        if threads_ > MAX_CORES:
            print(
                "Sorry, {} -- threads unavailable, using maximum CPU threads : {}".format(
                    threads_, MAX_CORES))
            threads_ = MAX_CORES
        ...
        tp.imap_unordered(lambda x: sanity_obj.get_img(x),
                          URL_ALL)  # pylint: disable= unnecessary-lambda #cSpell:words imap
        tp.close()

        # while(tp is not None):
        #     print("Waiting for thread to finish downloading satellite tiles")
        #     time.sleep(5)
        update_sanity_db(container_dir)
