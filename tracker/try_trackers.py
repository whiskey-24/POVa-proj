from tracker.src.deep_sort_rt import run_deep_sort_rt
from tracker.src.deep_sort import run_deep_sort
from tracker.src.strong_sort import run_strong_sort
from tracker.src.utils_tracker import load_dataset,create_video,evaluate_tracks
...
def main():
    ...
    if args.version == 1:
        ...
        create_video(processed_frames, output_file='deepsort_dataset.mp4', fps=5.0)
        ...
        tracker = DeepSort(
        max_iou_distance=0.6,    #adjusted for similarity in vehicle shapes/sizes
        max_age=40,              #slightly higher to account for potential occlusions
        n_init=5,                #standard initialization phase length
        nms_max_overlap=0.7,     #higher as overlap is less likely in aerial views
        ...
        bgr=False,                #match the color format of your input frames
       ...
       #tracker = DeepSort()
        ...
        create_video(processed_frames, output_file='deepsort_rt_dataset.mp4', fps=5.0)
        '''
        ...
        create_video(processed_frames, output_file='deepsort_rt_detector.mp4', fps=20.0)
